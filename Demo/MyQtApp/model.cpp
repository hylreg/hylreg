...
{
    // 连接信号和槽
    connect(this, &Model::startChanged, this, &Model::onModelPathChanged);
}

// 槽函数定义
void Model::onModelPathChanged() {
    // 执行你想做的事情，比如打印日志
    qDebug() << "Model path changed to:" << modelPath;
    // 你可以在这里添加更多的处理逻辑
    // yolov8ORT(imageProvider.frame);
}


std::vector<std::string> Model::readClassNames(std::string labels_txt_file)
{
...
void Model::yolov8ORT(cv::Mat &frame ,std::string onnxpath,std::string labels_txt_file)
{
...
QString Model::getModelPath() const
{
    return modelPath;
}

void Model::setModelPath(const QString &newModelPath)
{
...
QString Model::getClassNamePath() const
{
    return classNamePath;
}

void Model::setClassNamePath(const QString &newClassNamePath)
{
    if (classNamePath == newClassNamePath)
        return;
    classNamePath = newClassNamePath;
    emit classNamePathChanged();
}
...
void Model::setStart(int newStart)
{
    if (start == newStart)
        return;
    start = newStart;
