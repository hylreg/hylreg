...
showHelp (char *filename)
{
  ...
  std::cout << "     --algorithm (Hough|GC): 使用的聚类算法（默认 Hough）." << std::endl;
  std::cout << "     --model_ss val:         模型均匀采样半径（默认 0.01）" << std::endl;
  std::cout << "     --scene_ss val:         场景均匀采样半径（默认0.03）" << std::endl;
  std::cout << "     --rf_rad val:           参考帧半径（默认为 0.015）" << std::endl;
  std::cout << "     --descr_rad val:        半径描述符（默认为 0.02）" << std::endl;
  std::cout << "     --cg_size val:          群集大小（默认为 0.01）" << std::endl;
  std::cout << "     --cg_thresh val:        聚类阈值（默认值为 5）" << std::endl << std::endl;
}
...
main (int argc, char *argv[])
{
  ...
  for (std::size_t i = 0; i < scene_descriptors->size (); ++i)
  {
    ...
    if(found_neighs == 1 && neigh_sqr_dists[0] < 0.25f) //  仅当平方描述符距离小于 0.25 时才添加匹配（SHOT 描述符距离介于 0 和 1 之间 design)
    {
